...
namespace GBRead
{
	public class SyntaxRichTextBox : System.Windows.Forms.RichTextBox
	{
		...
		private StringBuilder m_strKeywords = new StringBuilder();
		private int m_nCurSelection = 0;
		private Regex m_regKeywords;
		private Regex m_regStrings;
		private Regex m_regIntegers;
		private Regex m_regComments;
		...
		private void ProcessLine()
		{
			...
			ProcessRegex(m_regKeywords, Settings.KeywordColor);
			// Process numbers
			if (Settings.EnableIntegers)
			{
				ProcessRegex(m_regIntegers, Settings.IntegerColor);
			}
			// Process strings
			if (Settings.EnableStrings)
			{
				ProcessRegex(m_regStrings, Settings.StringColor);
			}
			// Process comments
			if (Settings.EnableComments && !string.IsNullOrEmpty(Settings.Comment))
			{
				ProcessRegex(m_regComments, Settings.CommentColor);
			}
		...
		private void ProcessRegex(Regex strRegex, Color color)
		{
			    MatchCollection mtc = strRegex.Matches(m_strLine, 0);
		...
		public void CompileKeywords()
		{
			...
			for (int i = 0; i < Settings.Keywords.Count; i++)
			{
				...
				{
					m_strKeywords.AppendFormat("\\b{0}\\b", strKeyword);
				}
				else
				{
					m_strKeywords.AppendFormat("\\b{0}\\b|", strKeyword);
			...
			m_regKeywords = new Regex(m_strKeywords.ToString(), RegexOptions.IgnoreCase | RegexOptions.Compiled);
			m_regStrings = new Regex("\"[^\"\\\\\\r\\n]*(?:\\\\.[^\"\\\\\\r\\n]*)*\"", RegexOptions.IgnoreCase | RegexOptions.Compiled);
			m_regIntegers = new Regex("\\b(?:[0-9]*\\.)?[0-9]+\\b", RegexOptions.IgnoreCase | RegexOptions.Compiled);
			m_regComments = new Regex(Settings.Comment + ".*$", RegexOptions.Compiled | RegexOptions.IgnoreCase);
		}
