...
{
	public class GBBinFile : BinFile
	{
		...
		public GBBinFile() : base()
		{

		}

		public GBBinFile(byte[] rFile) : base(rFile)
		{

		}

		public override string GetBinInfo()
		{
			if (binFile.Length < 0x150) return "Not a properly headered GB/C file.";
			...
			if (mCode.ToString() != String.Empty) info.AppendLine("Manufacturer Code: " + mCode.ToString());
		...
		{
			if (!FileLoaded || binFile.Length < 0x150) return Int32.MinValue;
			ushort checksum = 0;
		...
		private string GetLicenseCode()
		{
			...
			else return ReadByte(oldLicenseCode).ToString("X2");
		}
